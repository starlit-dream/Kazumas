...
class _PlayerItemState extends State<PlayerItem>
  ...
  Future<void> _syncBangumiProgressStateForCurrentEpisode() async {
    ...
    try {
      _episodeWatchedReported = await BangumiSyncService().isEpisodeWatched(
        subjectId: videoPageController.bangumiItem.id,
        episodeNumber: videoPageController.playbackEpisode.episode,
      );
    } catch (error) {
      KazumiLogger().w(
        'Bangumi: failed to sync episode progress state',
        error: error,
      );
    } finally {
  ...
  Future<void> _markEpisodeWatched(
    ...
    try {
      final marked = await BangumiSyncService().markEpisodeWatched(
        subjectId: videoPageController.bangumiItem.id,
      ...
      _restoreWatchedRetryState(attemptEpisodeIdentity);
      KazumiLogger().w('Bangumi: failed to sync watched episode', error: error);
    }
  }
  ...
  void _maybePromptFinishReview() {
    ...
    final totalEpisodes =
        videoPageController.roadList[selection.road].data.length;
    final subjectId = videoPageController.bangumiItem.id;
    if (!FinishReviewTrigger.I.shouldPromptAfterEpisode(
      subjectId: subjectId,
  ...
  Timer getPlayerTimer() {
    return Timer.periodic(const Duration(seconds: 1), (timer) {
      final currentEpisodeNumber = videoPageController.playbackEpisode.episode;
      if (currentEpisodeNumber != _lastEpisodeNumber) {
        _lastEpisodeNumber = currentEpisodeNumber;
      ...
      if (_watchedAutoRecord &&
          ...
          !videoPageController.isOfflineMode &&
          BangumiAuth.isLoggedIn &&
          playerController.playback.playerDuration.inMilliseconds > 0) {
        final progress =
            playerController.playback.playerPosition.inMilliseconds /
                playerController.playback.playerDuration.inMilliseconds;
        if (progress >= _watchedAutoRecordThreshold) {
          final attemptEpisodeIdentity = _currentEpisodeIdentity;
  ...
  void initState() {
    ...
    _watchedPopupEnabled = GStorage.setting
        .get(SettingBoxKey.watchedPopupEnabled, defaultValue: true);
    _watchedAutoRecord = GStorage.setting
        .get(SettingBoxKey.watchedAutoRecord, defaultValue: false);
    _watchedAutoRecordThreshold = GStorage.setting.get(
