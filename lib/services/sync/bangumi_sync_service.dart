...
class BangumiSyncService {
  ...
  /// Returns whether the selected episode is already marked as watched.
  ///
  /// Episode-state requests stay in the synchronization layer so player UI
  /// code does not issue Bangumi HTTP calls directly.
  Future<bool> isEpisodeWatched({
    ...
    return _runExclusive(() async {
      final episode = await BangumiApi.getBangumiEpisodeByID(
        subjectId,
        episodeNumber,
      );
      if (episode.id == 0) {
      ...
      return await BangumiHTTP.getEpisodeCollectionType(episode.id) == 2;
    });
  }

  /// Marks one Bangumi episode as watched and confirms the remote state.
  Future<bool> markEpisodeWatched({
    ...
    return _runExclusive(() async {
      ...
      await BangumiHTTP.markEpisodeWatched(
        subjectId: subjectId,
